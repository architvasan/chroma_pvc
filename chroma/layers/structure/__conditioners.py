...
class SubsequenceConditioner(Conditioner):
    ...
    def forward(
        ...
        logp_S = checkpoint(self.checkpointed_operation, X_input, C, t, priority)
        if self.mask_condition is not None:
            logp_S = self.mask_condition * logp_S
        U = U + self.weight * -logp_S.sum()
        return X, C, O, U, t
    
    def checkpointed_operation(self, X_input, C, t, priority):
...
class ProCapConditioner(Conditioner):
    ...
    def forward(
        ...
        loss = checkpoint(self.checkpointed_operation, X, C, O, t)
        U = U + self.weight * loss
        return X, C, O, U, t
    def checkpointed_operation(self, X, C, O, t):
        return self.model(
            X,
            C,
...
class SubstructureConditioner(Conditioner):
    ...
    def forward(
        ...
        X = checkpoint(self.checkpointed_operation, Z)
        ...
        U = U + loss
        return X, C, O, U, t
    def checkpointed_operation(self, Z):
        return self.conditional_distribution.sample(Z=Z)
